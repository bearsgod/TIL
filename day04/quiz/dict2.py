...
# 난이도** 2. python standard library에 'requests'가 있나요? : 접근 및 list in
search = ssafy['language']['python']['python standard library']
print('requests' in search)
# if 'requests' in search:
#     print('true')
# else:
#     print('false')
...
# 난이도*** 4. ssafy에서 배우는 언어들을 출력하세요. : dictionary.keys() 반복
lang = ssafy['language'].keys()
for i in lang:
    print(i)

# 난이도*** 5 ssafy gj반의 강사와 매니저의 이름을 출력하세요. dictionary.values() 반복
gjmem = ssafy['classes']['gj'].values()
for j in gjmem:
    print(j)

# 난이도***** 6. framework들의 이름과 설명을 다음과 같이 출력하세요. : dictionary 반복 및 string interpolation
fwinfo = ssafy['language']['python']['frameworks'].items()
for name, info in fwinfo:
    print(f'{name}은 {info}이다.')

# 난이도***** 7. 오늘 당번을 뽑기 위해 groups의 4조 중에 한명을 랜덤으로 뽑아주세요. : depth 있는 접근 + list 가지고 와서 random.
ranmem = random.choice(ssafy['classes']['gm']['groups']['4조'])
print(f'오늘의 당번은 {ranmem}')